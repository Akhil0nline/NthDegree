...
const advanceValues = [100,200,300];


test('Advance calculation on Price List Tool should be correctly displayed', async({page}) => 
{   
...
for (let i = 1; i <= advanceValues.length; i++) {
    await page1.locator('td[name="ProductAdvance"]').nth(i).click();
    await page1.locator('td[name="ProductAdvance"]').nth(i).fill(advanceValues[i - 1].toString());
