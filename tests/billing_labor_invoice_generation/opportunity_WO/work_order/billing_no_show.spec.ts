...
test('Verify that Billing Rate List Overridden is set to No when Billing Rate List matches the Show Labor Billing Rate.', async ({ page }) => {
    ...
    console.log('Show Setup Price List:', showSetupBilling);
    ...
    await page.getByLabel('Billing', { exact: true }).click();
    await page.waitForTimeout(5000);
    const billingrate_overriden = await page.locator("//div[@title='Fern - Las Vegas']").innerText();
    const Value_overriden = await page.locator("(//input[@value='No'])[2]").inputValue();
    await page.waitForTimeout(3000);
    if (billingrate_overriden === showSetupBilling) {
