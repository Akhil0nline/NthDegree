...
test(' Verify that Charge for Order defaults to Yes when Line of Business does not begin with "Fern"', async ({ page }) => {
    ...
    expect.soft(await page.getByRole('option', { name: 'International' }).click());
    ...
    opportunityNumber = opportunityNumber.replace('- Saved', '').trim();
    expect(opportunityNumber.toLowerCase()).toContain("o-24");
